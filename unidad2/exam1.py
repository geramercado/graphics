...
pygame.display.set_caption("Fractales examen U2 Gerardo Mercado Hurtado/ Agustin Paniagua Flores")
...
def dibujar_sierpinski_cuadrado(superficie, nivel, x, y, tamaño, color):
    # Si es el último nivel, dibujar el cuadrado sólido
    if nivel == 0:
        pygame.draw.rect(superficie, color, (x, y, tamaño, tamaño))
    else:
        nuevo_tamaño = tamaño // 3
        for i in range(3):
            for j in range(3):
                # Saltar el centro (i==1 y j==1)
                if i == 1 and j == 1:
                    continue
                dibujar_sierpinski_cuadrado(
                    superficie,
                    nivel - 1,
                    x + i * nuevo_tamaño,
                    y + j * nuevo_tamaño,
